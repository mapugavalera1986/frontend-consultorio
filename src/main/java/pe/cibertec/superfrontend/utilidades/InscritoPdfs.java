...
public class InscritoPdfs implements IGenerarPdfs<Inscrito> {
	...
	@Override
	public void crearGrupal(List<Inscrito> listado, HttpServletResponse respuesta)
		...
		Paragraph encabezado = new Paragraph("Lista de participantes inscritos", elegirFuente(16, "bold"));
		encabezado.setAlignment(Paragraph.ALIGN_JUSTIFIED);
		encabezado.setSpacingAfter(12);
		nuevo_doc.add(encabezado);
		for (Inscrito i : listado) {
	...
	private Paragraph redactarEntidad(Inscrito entidad, Font fuente) {
		String texto_entidad = "Participante: " + entidad.getNmbrs() + " " + entidad.getApllds() + "\nFecha nac.: "
				...
				+ "\nModalidad: " + entidad.getModalidad();
